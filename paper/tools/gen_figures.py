...
# The retrained in-mode probe in the MLP-branch space is the configuration in which the probe reads UNENCODED text best
# (highest median length-stratified AUC on plain text and on leetspeak), so it is the one the figure shows: the claim is
# that even the strongest reader of readable text fails on Base64/ROT13. It is NOT the configuration holding the single
# best Base64 or leetspeak cell -- both of those lie in the residual space (Appendix A, panel C) -- and the caption says
# so. The caption's numbers are written by this generator to figures/_fig_obfuscation_numbers.tex, never typed.
...
def fig_obfuscation():
    ...
    modes = ["raw", "templated"]
    fig, axes = plt.subplots(2, 1, figsize=(7.0, 2.6), constrained_layout=True)
    im = None
    ...
    _fig_obfuscation_numbers(rel)
    return out.name


def _fig_obfuscation_numbers(rel):
    """Every number the Figure 2 caption quotes, as LaTeX macros, from the same file the figure is drawn from."""
    import numpy as np
    feats = json.load(open(src(rel)))["features"]
    shown = feats[FEATURE]["cells"]
    def cells(cond):
        return [c["auc_length_stratified"] for k, c in shown.items()
                if k.split("|")[0] == PROBE and k.split("|")[2] == WRAPPER and k.endswith("|" + cond)
                and c["auc_length_stratified"] is not None]
    enc = cells("base64") + cells("rot13")
    best = {}
    for cond in ("base64", "leetspeak"):
        best[cond] = max(((c["auc_length_stratified"], f) for f in feats for k, c in feats[f]["cells"].items()
                          if k.endswith("|" + cond) and c["auc_length_stratified"] is not None), key=lambda t: t[0])
    spaces = {best[c][1] for c in best}
    if spaces != {"residual"}:
        from gen_tables import MissingSource
        raise MissingSource(f"gen_figures: the Figure 2 caption says the best Base64 and leetspeak cells lie in the residual "
                            f"space; the data now puts them in {spaces}. Revise the caption in sections/07_obfuscation.tex.")
    macros = {"obfShownPlainMedian": f"{np.median(cells('none')):.3f}", "obfShownLeetMedian": f"{np.median(cells('leetspeak')):.3f}",
              "obfShownEncMedian": f"{np.median(enc):.3f}", "obfShownEncMax": f"{max(enc):.3f}", "obfShownEncCells": str(len(enc)),
              "obfBestBaseSixtyFour": f"{best['base64'][0]:.3f}", "obfBestLeet": f"{best['leetspeak'][0]:.3f}", "obfBestSpace": "residual"}
    lines = ["% GENERATED by paper/tools/gen_figures.py -- do not edit by hand. Numbers quoted by the Figure 2 caption.",
             f"% source: {rel}"] + [rf"\newcommand{{\{k}}}{{{v}}}" for k, v in macros.items()]
    (FIGURES / "_fig_obfuscation_numbers.tex").write_text("\n".join(lines) + "\n")
...
EXPECTED = ["fig_extraction.pdf", "fig_obfuscation.pdf", "_fig_obfuscation_numbers.tex"]
