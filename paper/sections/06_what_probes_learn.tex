...
the same thing more bluntly: $0$--$17\,\%$ on XSTest against $30$--$94\,\%$ on the on-topic set, at each probe's own shipped threshold; at the
matched operating point of Table~\ref{tab:extraction_convention} the same probes reach $0.99$. A detector that fires
...
Refusal separation says the same thing from the model's side. Table~\ref{tab:af_refusal} conditions the shipped
probe's flag rate on whether the model's own continuation was a refusal. On HarmBench the probe flags $0.91$--$1.00$ of
the prompts six of the seven models refuse anyway, so on the harmful side it is largely redundant with the model's
behaviour; the exception, Gemma-3-4B, flags about a third in either condition. On the on-topic benign set it flags
answered prompts at $0.69$--$0.96$ on those same six models, refusal or no refusal. A detector that fires on what the
model already refuses and on benign requests the model answers is tracking the subject, not the decision.

\input{tables/tab_af_refusal}
...
Our own earlier release \cite{messenger2026aase} proposed, as others have for minimal-data probes generally, to build
the training set from matched pairs---each positive aligned with a negative sharing topic, style and length---and to
average the \emph{within-pair} differences, on the argument
...
We report this as a retirement of our own proposal rather than as a refutation of it. The proposal is not wrong about pairing cancelling shared
...
(Table~\ref{tab:apc_refusal}). The refusal band holds $20$ responses, so this is an illustration of the conflation
rather than an estimate of its rate. The perfect original score is an artifact of a six-example split; the refusal result
...
\input{tables/tab_apc_external}
\input{tables/tab_apc_refusal}
...
\input{tables/tab_apc_construction}
...
\textbf{The finding.} The three failures share one cause, which we state as a result rather than as a limitation, and
inductively---on the evidence of three instruments, not by proof: in
...
provide. The lexical baseline's role here is as a validity bound on the evaluation rather than as a recommended
control, and none of this licenses regex-based enforcement on longer responses, where the cues and the construct are
precisely what we expect to come apart. Second, certifying semantic speech-act classification requires naturally occurring multi-sentence responses,
