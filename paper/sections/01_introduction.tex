...
topic-matched benign prompts become accurate detectors (length-stratified AUC $0.92$--$0.98$, exceeding tuned lexical
baselines by $+0.21$ to $+0.27$) when prompts are read through the model's chat template rather than as bare text; and
probes that the conventional final-token read leaves at $0.58$--$0.86$ reach $0.94$--$0.97$ AUC when the activation is
instead averaged over the tool-response span, on injections never seen in training.
...
\item \textbf{Two extraction-convention repairs.} Reading prompts through the model's chat template removes the topic
      ...
      from $0.58$--$0.86$ to $0.94$--$0.97$ AUC on injections never seen in training (Section~\ref{sec:extraction}).
\item \textbf{Retirement of paired contrastive training.} Its estimator is algebraically identical to standard
      contrastive training for complete aligned pairs, and an ablation holding the estimator fixed shows its data
      construction contributes between $-0.003$ and $+0.018$ AUC (Section~\ref{sec:learn}).
...
\item \textbf{An anatomy of obfuscation failure.} Across $22$ blocks, two template modes, two probe constructions and
      two activation spaces, no configuration separates encoded-harmful from encoded-benign prompts once length is
      controlled---even for encodings the model demonstrably decodes---while training on encoded examples
