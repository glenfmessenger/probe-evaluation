"""
Agent Action Gating - Phase 1 v4: Multi-Vector Approach
========================================================
Addresses the observation that different attack types have different
activation signatures. Uses specialized vectors for:
1. ACTION vectors - unauthorized actions on external resources
2. DISCLOSURE vectors - revealing protected information
3. IDENTITY vectors - misrepresenting identity
...
# =============================================================================
# TRAINING DATA - Organized by Attack Type
# =============================================================================

# TYPE 1: ACTION VIOLATIONS - Unauthorized actions on external resources
ACTION_TRAINING = [
    {
...
# TYPE 2: DISCLOSURE VIOLATIONS - Revealing protected information
DISCLOSURE_TRAINING = [
    {
...
# TYPE 3: IDENTITY VIOLATIONS - Misrepresenting identity or role
IDENTITY_TRAINING = [
    {
        "id": "identity_company",
...
def classify_with_vectors(
    activation: np.ndarray,
    vectors: Dict[str, DirectionVector],
    attack_type: Optional[str] = None
) -> Tuple[bool, float, str, Dict]:
    """
    Classify using appropriate vector(s).
    If attack_type is specified, use that vector.
    Otherwise, use maximum score across all vectors.
    """
    scores = {}
    
    for name, dv in vectors.items():
        score = np.dot(activation, dv.vector)
        is_unsafe = score > dv.threshold
        margin = score - dv.threshold
        scores[name] = {
            "score": float(score),
    ...
    if attack_type and attack_type in vectors:
        # Use specific vector
        result = scores[attack_type]
        return result["is_unsafe"], result["score"], attack_type, scores
    else:
        # Use max margin (most confident unsafe signal)
        max_margin_type = max(scores.keys(), key=lambda k: scores[k]["margin"])
        result = scores[max_margin_type]
        return result["is_unsafe"], result["score"], max_margin_type, scores
...
def run_benchmark(model, tokenizer, vectors: Dict[str, DirectionVector]) -> List[TestResult]:
    ...
    for case in BENCHMARK_CASES:
        ...
        print(f"\n{'='*60}")
        print(f"Case: {case_id} | Type: {attack_type} | Attack: {is_attack}")
        
        # Score safe action
        safe_prompt = format_prompt(case["system"], case["tool_output"], case["safe_action"])
        safe_activation = extract_activation(model, tokenizer, safe_prompt)
        
        # Use type-specific vector if attack, otherwise use max
        vector_to_use = attack_type if attack_type != "none" else None
        safe_is_unsafe, safe_score, safe_vector, safe_all_scores = classify_with_vectors(
            safe_activation, vectors, vector_to_use
        )
        
        print(f"  Safe action: {safe_score:.2f} ({safe_vector}) -> {'UNSAFE' if safe_is_unsafe else 'SAFE'}")
        ...
        if case["unsafe_action"]:
            ...
            unsafe_is_unsafe, unsafe_score, unsafe_vector, unsafe_all_scores = classify_with_vectors(
                unsafe_activation, vectors, vector_to_use
            )
            
            print(f"  Unsafe action: {unsafe_score:.2f} ({unsafe_vector}) -> {'UNSAFE' if unsafe_is_unsafe else 'SAFE'}")
        ...
        results.append(TestResult(
            ...
            vector_used=safe_vector,
            all_scores=safe_all_scores
        ))
...
def main():
    print("="*70)
    print("AGENT ACTION GATING - PHASE 1 v4: MULTI-VECTOR APPROACH")
    print("="*70)
    ...
    # Comparison
    print(f"\n--- Comparison ---")
    print(f"v2: 70.6%, v3: 76.5%, v4: {metrics['overall_accuracy']*100:.1f}%")
    ...
    import os
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "phase1_v4_results.json")
