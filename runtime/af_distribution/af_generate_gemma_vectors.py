...
class VectorTrainer:
    ...
    def train_category(self, category: str, positive: List[str], negative: List[str]) -> CategoryStats:
        ...
        # Use percentile-based threshold for robustness across different model scales
        # Find threshold where 95% of negatives are below and 95% of positives are above
        neg_95 = np.percentile(neg_scores, 95)
        pos_5 = np.percentile(pos_scores, 5)
        
        # If there's good separation, use the midpoint of the gap
        # If overlap, use a weighted threshold favoring fewer false positives
        if pos_5 > neg_95:
    ...
    def train_obfuscation(self) -> CategoryStats:
        ...
        # Use percentile-based threshold for robustness across model scales
        neg_95 = np.percentile(neg_scores, 95)
        pos_5 = np.percentile(pos_scores, 5)
        ...
        if pos_5 > neg_95:
            ...
            # Be conservative for obfuscation
            threshold = np.percentile(neg_scores, 98)
        
        # High-confidence threshold: only trigger if clearly obfuscated
        neg_99 = np.percentile(neg_scores, 99)
        pos_25 = np.percentile(pos_scores, 25)
        high_conf_threshold = max(neg_99, (neg_99 + pos_25) / 2)
        ...
        print(f"      Threshold: {threshold:.2f}, High-conf: {high_conf_threshold:.2f}")
        print(f"      Separation: {separation:.2f}σ, Accuracy: {train_acc:.1%}, FP: {fp_rate:.1%}")
        print(f"      High-conf FP: {hc_fp/len(neg_scores):.1%}, FN: {hc_fn/len(pos_scores):.1%}")
