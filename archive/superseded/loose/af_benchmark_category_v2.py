"""
AF Benchmark: Category-Specific Vectors for AILuminate (v2.0)
=============================================================
...
class AFCategoryClassifier:
    ...
    def train_obfuscation_vector(self) -> CategoryStats:
        """
        v2: Train obfuscation meta-vector with expanded data and conservative threshold.
        """
        print(f"    Training obfuscation meta-vector v2 (sensitivity={self.obf_sensitivity})...", end=" ")
        ...
        # sensitivity=1.0 -> balanced midpoint (v1 behavior)
        # sensitivity=0.7 -> closer to negative mean (fewer FP)
        ...
        print(f"    separation={separation:.2f}σ, acc={train_acc:.1%}, FP={fp_rate:.1%}, FN={fn_rate:.1%}")
    ...
    def is_blocked(self, prompt: str, enabled_categories: List[str] = None) -> Tuple[bool, List[str], Dict[str, float], float]:
        """
        v2: Check if prompt should be blocked with optional confirmation logic.
        
        Two-stage check:
        1. Obfuscation meta-vector (catches encoded attacks)
        2. Category vectors (catches plain-text policy violations)
        
        If require_confirmation=True (default in v2):
            Obfuscation only blocks when at least one category also triggers.
            This prevents blocking benign technical content that looks encoded.
        """
        start = time.perf_counter()
        ...
        # v2: Decision logic with optional confirmation
        if self.require_confirmation:
            # Obfuscation only blocks if a category also triggers
            ...
            # This is the key change that reduces FP on benign technical content
        else:
...
class CategoryAFBenchmark:
    ...
    def __init__(self, gemma_model, gemma_tokenizer, 
        ...
        print(f"    AF config: layer_pct={af_layer_pct}, obf_sensitivity={obf_sensitivity}, require_confirmation={require_confirmation}")
    ...
    def evaluate_af(self, prompts: List[AILuminatePrompt]) -> Tuple[BenchmarkSummary, List[BenchmarkResult]]:
        ...
        print(f"\n{'='*70}")
        print("CONFIG: Gemma + AF v2 (Category Vectors)")
        print("="*70)
        ...
        summary = self._compute_summary(results, "Gemma + AF v2")
        return summary, results
...
def run_benchmark(args):
    ...
    print("=" * 70)
    print("AF CATEGORY BENCHMARK v2.0")
    print("=" * 70)
    print(f"\nv2 Features:")
    print(f"  - Expanded obfuscation training data (60 neg vs 20)")
    print(f"  - Conservative threshold with sensitivity={args.obf_sensitivity}")
    ...
    # Initialize benchmark with v2 options
    ...
    output_file = "af_category_benchmark_v2_results.json"
...
def main():
    parser = argparse.ArgumentParser(description="AF Category-Specific Benchmark v2")
