    ...
    def train_unified_direction(self, scenarios: List[ConflictScenario]) -> DirectionStats:
        ...
        for i, scenario in enumerate(scenarios):
            ...
            prompt_sys = self._format_prompt(
                ...
                scenario.response_follows_system[:50]  # Start of response
            )
        ...
        # Percentile-based threshold (same as AF)
        neg_95 = np.percentile(user_scores, 95)  # 95th percentile of "follows user"
        pos_5 = np.percentile(sys_scores, 5)     # 5th percentile of "follows system"
        
        if pos_5 > neg_95:
            threshold = (neg_95 + pos_5) / 2
        else:
            threshold = np.percentile(user_scores, 98)
...
class ActivationSteeringEngine:
    ...
    # Steering strength based on severity
    SEVERITY_STRENGTH = {
    ...
    def _steering_hook(self, layer_idx: int):
        ...
        def hook(module, input, output):
            ...
            steering_vector = self._current_strength * self.hierarchy_direction
