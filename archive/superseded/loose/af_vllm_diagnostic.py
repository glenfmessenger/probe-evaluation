...
def extract_vllm_activation(prompt: str, layer_idx: int) -> Tuple[np.ndarray, str, int]:
    ...
    # Register hook
    activation_data = {"activation": None, "shape": None}
    ...
    def register_hook(model):
        ...
        def hook_fn(module, input, output):
            ...
            activation_data["shape"] = hidden.shape
            activation_data["activation"] = hidden[-1, :].detach().float().cpu().numpy()
            # Also save full tensor info for debugging
            np.save("/tmp/af_vllm_full_hidden.npy", hidden.detach().float().cpu().numpy())
    ...
    hidden_shape = activation_data["shape"]
    ...
    # Check if hidden tensor has more tokens than expected
    if hidden_shape[0] != seq_len:
        print(f"[vLLM] WARNING: Hidden shape {hidden_shape[0]} != token count {seq_len}")
        print(f"[vLLM] This suggests the -1 index may not be the last input token!")
