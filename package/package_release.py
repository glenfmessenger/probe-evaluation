...
def create_probes(output_dir: Path):
    ...
    # Probes __init__
    probes_init = '''from .base import BaseProbe, ProbeResult
from .af import ActivationFingerprint
from .aag import AgentActionGating
from .apc import ActivationPolicyCompliance
'''
...
class SafetyGate:
    ...
    def load_pretrained(
        ...
        if enable_af:
            af_path = base / "af" / f"{model_safe}.npy"
            if af_path.exists():
                self.add_probe(ActivationFingerprint.load(str(af_path.with_suffix("")), "af"))
        
        if enable_aag:
            aag_path = base / "aag" / f"{model_safe}.npy"
            if aag_path.exists():
                self.add_probe(AgentActionGating.load(str(aag_path.with_suffix("")), "aag"))
        
        for policy in apc_policies:
            apc_path = base / "apc" / f"{model_safe}_{policy}.npy"
            if apc_path.exists():
                self.add_probe(ActivationPolicyCompliance.load(str(apc_path.with_suffix("")), f"apc_{policy}"))
