...
class ConfidenceModel:
    ...
    # Using Q&A format to match generation context
    CALIBRATION_DATA = [
    ...
    def __init__(
        ...
        scale_factor: float = 15.0,  # Tuned for Q&A calibration (scores range -30 to +50)
    ):
        """
        ...
        Args:
            ...
            scale_factor: Normalization scale for sigmoid (default 15.0, tuned for Q&A)
        """
        self.model = model
...
def demo():
    ...
    # NOTE: AHD measures model confidence, not correctness
    # High = model is confident (may be right or wrong)
    # Low = model is uncertain (usually refuses/hedges)
    test_cases = [
        # High confidence - model knows these
        ("What is the capital of France?", "high"),
        ("Who wrote Romeo and Juliet?", "high"),
        ("What is the atomic number of gold?", "high"),
        
        # High confidence - model will answer confidently (even if wrong!)
        ("What is the national bird of Lesotho?", "high"),  # May hallucinate confidently
        
        # Low confidence - model should refuse/hedge
        ("Who is Dr. Marcus Wellby, the Stanford professor?", "low"),
        ("What is the plot of 'The Chromatic Paradox' by Jennifer Ashworth?", "low"),
    ]
